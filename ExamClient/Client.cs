    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                client.Send(Encoding.UTF8.GetBytes(joinMessage));

                byte[] data = new byte[1024];

                int size = client.Receive(data);

                string response = Encoding.UTF8.GetString(data, 0, size);
                ...
                while (true){
                    ...
                    if (parts[0] == "QUESTION"){
                        ...
                        Console.WriteLine("CAU HOI SO " + parts[1]);
                        ...
                        string answerPacket ="ANSWER|" + parts[1] + "|" + answer;

                        client.Send(Encoding.UTF8.GetBytes(answerPacket));

                        Console.WriteLine("Da gui dap an.");
                    }

                    else if (parts[0] == "RESULT"){
                        Console.WriteLine();
                        Console.WriteLine("Ket qua: " + parts[1]);
                    }
                    ...
                    else if (parts[0] == "RANKING")
                    {
                        ...
                        Console.WriteLine();
                        Console.WriteLine("Ky thi da ket thuc.");
                        break;
                    }
                }
                client.Close();
